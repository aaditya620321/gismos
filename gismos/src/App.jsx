  ...
  return (
    <div className="top-bar">
      ...
      <ul className="nav-bar">
        <li className="nav-link">
          <a
            onClick={() => {
              ...
              navigate(userDetails._id ? config.ordersPage : config.loginPage);
            }}
          >
            <span>
              <svg
        ...
        <li className="nav-link">
          <a
            onClick={() => {
              document.getElementById("cart-view").classList.remove("visible");
              navigate(userDetails._id ? config.confirmPage : config.loginPage);
            }}
          >
            <span>
              <svg
      ...
      <span className="right-nav">
          ...
          >
            <span>
              <svg
                ...
                height="32"
                version="1.1"
                viewBox="0 0 47.967 48.049"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="m23.054 37.992c0 1.77-1.543 3.2048-3.4464 3.2048s-3.4464-1.4348-3.4464-3.2048c-1e-6 -1.77 1.543-3.2048 3.4464-3.2048s3.4464 1.4348 3.4464 3.2048zm-6.1096-9.3234s-4.4386 0-4.3118 2.7192c0.1268 2.7192 4.5654 2.7192 4.5654 2.7192l24.73-0.1648m-40.328-26.958s5.1162-0.65174 6.9069 0.93104c1.7907 1.5828 7.9301 19.925 7.9301 19.925s-0.63952 0.83792 4.7325 0.93104c5.372 0.0931 18.674 0.37242 20.337-0.37242 1.6628-0.74485 4.8604-13.128 4.8604-13.128s-0.63954-2.9794-4.9883-3.2587c-4.3488-0.27932-25.197 0.0931-25.197 0.0931s-3.8372 0.74485-4.2209 1.6759m28.513 24.21c0 1.77-1.5063 3.2048-3.3643 3.2048s-3.3643-1.4348-3.3643-3.2048c0-1.77 1.5063-3.2048 3.3643-3.2048s3.3643 1.4348 3.3643 3.2048z"
                  fill="none"
                  stroke="#000"
                  strokeLinecap="round"
                  strokeLinejoin="round"
        ...
        <div className="nav-bar-button nav-link">
          <a
            onClick={() => {
              document
          ...
          >
            <span>
              <img src="./navbutton.png" alt="▼" height={24} width={24} />
            </span>
          </a>
        </div>
      </span>
...
//         axios.post(config.getBackendUrl("/add-product"),{...product, lol:"$MiLl2025$"},{})
//         .then(res =>{
...
//   }
  ...
  function removeFromCart(product_id) {
    ...
    axios
      .put(
        config.getBackendUrl("/update-cart"),
        { cart: cart },
        {
          headers: {
      ...
      .catch((err) => {
        toast.error(
          err.response ? err.response.data.message : "Some error occurred"
        );
      });
  }
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .post(
          config.getBackendUrl("/autologin-gismos"),
          {},
          {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          Component={() => {
            return (
              <>
                ...
                <ProductPage
                  userDetails={userDetails}
                  currentProduct={currentProductView}
                  addToCart={addToCart}
                  userCart={userCart}
                />
              </>
